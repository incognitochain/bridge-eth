...
task("list-contracts", "Exports & prints the list of deployed contracts")
    .setAction(async taskArgs => {
        ...
        console.log(result);
        const { getImplementation, getInstance, getCodeSlot } = require('./scripts/utils');
...
task("vault-balance-status", "Balance by token")
    .addParam("token", "The ERC20 token")
    .setAction(async taskArgs => {
        const { getInstance } = require('./scripts/utils');
        let token = taskArgs.token;
        if (!token.length) token = '0x0000000000000000000000000000000000000000';

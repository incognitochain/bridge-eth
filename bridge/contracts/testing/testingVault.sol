...
contract TestingVault {
    ...
    */
    ChiToken public chi;
    // toggle for testing; this contract will behave maliciously when attacked
    ...
    */
    ...
    /**
     * @dev Change the CHI contract address for testing
     */
     function setChi(address c) external {
        chi = ChiToken(c);
     }
    ...
    modifier discountCHI {
        ...
        if (!address(chi).isContract()){
            emit Debug("CHI address is not a contract !");
        } else {
            try chi.freeFromUpTo(msg.sender, (gasSpent + 14154) / 41947) {
                emit Debug("Freeing CHI succeeded !");
            }
            catch{
                emit Debug("Freeing CHI failed !");
                /**
...
contract WrongChi {
    ...
    function doAny(string memory thing) public {
        emit Announce(thing);
    }
}

// these functions have a public 'mint' function to ease testing

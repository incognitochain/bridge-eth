...
//         NOT_USE_PLG_EXECUTE_SIGNATURE,
//         NOT_USE_PLG_REQUEST_WITHDRAW_SIGNATURE,
...
//         require(data.meta == 155 && data.shard == 1, errorToString(Errors.INVALID_DATA)); // Check instruction type
...
//         require(inst.length >= 130, errorToString(Errors.INVALID_DATA));
//         BurnInstData memory data = parseBurnInst(inst);
//         require(data.meta == 156 && data.shard == 1, errorToString(Errors.INVALID_DATA)); // Check instruction type
...
//         address verifier = verifySignData(abi.encode(newPreSignData(Prefix.FTM_REQUEST_WITHDRAW_SIGNATURE, token,
//             timestamp, amount), incognitoAddress), signData);
...
//         address verifier = verifySignData(abi.encode(newPreSignData(Prefix.FTM_EXECUTE_SIGNATURE, token, timestamp,
//             amount), recipientToken, exchangeAddress, callData), signData);

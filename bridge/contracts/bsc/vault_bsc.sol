...
contract Vault {
    ...
    */
    ...
    enum Prefix {
        EXECUTE_SIGNATURE_NOT_USED,
        REQUEST_WITHDRAW_SIGNATURE_NOT_USED,
        EXECUTE_SIGNATURE_BSC,
        REQUEST_WITHDRAW_SIGNATURE_BSC
    }
    ...
    function submitBurnProof(
        ...
        require(inst.length >= 130, errorToString(Errors.INVALID_DATA));
        BurnInstData memory data = parseBurnInst(inst);
        require(data.meta == 255 && data.shard == 1, errorToString(Errors.INVALID_DATA)); // Check instruction type
    ...
    function requestWithdraw(
        ...
        address verifier = verifySignData(abi.encode(newPreSignData(Prefix.REQUEST_WITHDRAW_SIGNATURE_BSC, token, timestamp, amount), incognitoAddress), signData);
    ...
    function execute(
        ...
        address verifier = verifySignData(abi.encode(newPreSignData(Prefix.EXECUTE_SIGNATURE_BSC, token, timestamp, amount), recipientToken, exchangeAddress, callData), signData);

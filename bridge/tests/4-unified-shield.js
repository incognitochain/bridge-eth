...
const { unified_shield, unified_unshield, unified_shieldToken, unified_unshieldToken, burnForCall, burnForCallRefundedInstruction, burnForCallRefundedEvent, burnForCallWithdrawException, invalidFunctionSignature, changeByte, makeInvalidBurnParams } = require('./unified-token-helpers');
...
describe("Unified Shield & Unshield", async function() {
    ...
    describe.skip('pApp: swap upETH for pToken2', async function() {
    ...
    describe.skip('pApp: swap upToken2 for upETH', async function() {
    ...
    describe.skip('pApp: swap pToken1 for pETH', async function() {
    ...
    describe.skip('pApp: swap pETH for Token1', async function() {
        it(`should receive ${ethers.utils.formatUnits(startEth, 'gwei')} (gwei) ETH deposit`, shield(startEth));
        let amount = getPartOf(startEth, 50);
    ...
    describe('pApp: burn upETH -> refund', async function() {
        ...
        it(`refund event: calling nonexistent function`, burnForCallRefundedEvent('kyber', startEth, null, 'Token1', { useWithdraw: true, useUnified: { burn: true, reshield: true }, changeIndex: 3 }));
        it(`refund event: calling with changed inputs`, burnForCallRefundedEvent('kyber', startEth, null, 'Token1', { useWithdraw: false, useUnified: { burn: true, reshield: true }, changeIndex: 4 }));
    ...
    describe('pApp: burn upToken1 -> refund', async function() {
        ...
        it(`refund event: calling nonexistent function`, burnForCallRefundedEvent('kyber', startToken, 'Token1', null, { useWithdraw: true, useUnified: { burn: true, reshield: true }, changeIndex: 3 }));
        it(`refund event: calling with changed inputs`, burnForCallRefundedEvent('kyber', startToken, 'Token1', null, { useWithdraw: false, useUnified: { burn: true, reshield: true }, changeIndex: 4 }));
